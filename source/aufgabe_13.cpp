...
TEST_CASE()
{
  ...
  std::copy_if(circles.begin(), circles.end(), std::back_inserter(greater_four), 
              [] (Circle const& c) { return c.radius() > 4; });
 
  REQUIRE(std::all_of(greater_four.begin(), greater_four.end(), 
              [] (Circle const& c) { return c.radius() > 4; }));
}
